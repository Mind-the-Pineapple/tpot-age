...
def extendedeaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen, pbar,
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    if len(fitnesses_only) == 0:
        curr_min=0
        curr_max=0
        curr_std = 0
        curr_avg = 0
    else:
        curr_min = np.min(fitnesses_only)
        curr_max = np.max(fitnesses_only)
        curr_std = np.std(fitnesses_only)
        curr_avg = np.mean(fitnesses_only)
    logbook.record(gen=0, nevals=len(invalid_ind),
                   avg=curr_avg, std=curr_std,
...
def _fit_and_score(estimator, X, y, scorer, train, test, verbose,
    ...
    """Fit estimator and compute scores for a given dataset split.
        This original function has been copied from sklearn so that the training
        and test data are scaled using MixMaxScaler
